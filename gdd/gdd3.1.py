...
formats = {'mso':("doc","xls","ppt"),'oo':("odt","ods","ppt"),'pdf':("pdf","pdf","pdf"),'txt':("txt","csv","txt"),'html':("zip","html","swf"),}
format = "mso"
...
parser.add_option("--format",dest="format",help="Sets the format that documents are downloaded as")
parser.add_option("--document-format",dest="docformat",	help="Sets the format that word processor documents are downloaded as")
parser.add_option("--spreadsheet-format",dest="spdformat",help="Sets the format that spreadsheets are downloaded as")
parser.add_option("--presentation-format",dest="prsformat",help="Sets the format that presentations are downloaded as")
...
cfg = {'email':'','password':'','path':script_path,'format':'mso','feed':'https://docs.google.com/feeds/default/private/full','document_format':'','spreadsheet_format':'','presentation_format':'','use_unicode':False,'use_datetag':False,'use_folders':False,'modified_only':False,'clean_up':False,'illegal_characters':'<>:"/\|?*^'}
	...
	for key in cfg.keys():
		if config.has_option(options.profile, key):
			if config.get(options.profile, key) != "":
				if (key.find("use_") == 0 or key == "modified_only" or key == "clean_up"):
...
def getFile(doc,headers,dname,tries=0):
	...
	if cfg['modified_only'] and doc['id'] in etags and doc['etag'] == etags[doc['id']] and doc['id'] in prev_successfuldocs.keys() and os.path.exists(dname+fname):
