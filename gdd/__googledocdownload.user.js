...
var SCRIPT = {
	...
	version: "3.1",
	date: (new Date(2011, 09, 04)).valueOf()
};
...
function makepage(files){
	...
	linklist.write('<p style=\"margin-left:auto;margin-right:auto;width:480px;\"><select style=\"width:100%;\" onChange=\"if(this.value!=\'\'){for(var i = 0; i < document.links.length; i++){var x=document.links[i];x.href=x.href.replace(/exportFormat\\=ppt/ig,\'exportFormat=\'+this.value);x.href=x.href.replace(/exportFormat\\=txt/ig,\'exportFormat=\'+this.value);x.href=x.href.replace(/exportFormat\\=pdf/ig,\'exportFormat=\'+this.value);}}\"><option value=\'\'>Change Presentation Format</option><option value=\"ppt\">Microsoft Powerpoint Document</option><option value=\"txt\">Text Document</option><option value=\"pdf\">PDF Document</option></select></p>');
