...
var SCRIPT = {
	...
	date: (new Date(2008, 6, 14))		// update date
			.valueOf()
};
...
function GetVersion(force){
	GM_xmlhttpRequest({
		...
		onload: function(responseDetails) {
			...
			//alert("Get Version: Found remote version. " + remoteVersion);
			CheckVersion(force);
		}
	});
}
...
function CheckVersion(force){
	...
	//alert("Check version: " + remoteVersion + " > " + localVersion);
